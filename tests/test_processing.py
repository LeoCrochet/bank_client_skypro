...
@pytest.fixture
def transactions_with_same_date() -> list:
...
@pytest.fixture
def transactions_no_state() -> list:
...
def test_filter_by_state_executed(transactions: list) -> None:
...
def test_filter_by_state_canceled(transactions: list) -> None:
...
def test_filter_by_state_no_matching(transactions_no_state: list) -> None:
...
def test_filter_by_state_parametrized(transactions: list, state: str, expected_ids: list) -> None:
...
def test_filter_by_state_invalid_type() -> None:
    ...
    with pytest.raises(TypeError, match=f"transactions должен быть списком, получен "):
...
def test_sort_by_date_same_dates(transactions_with_same_date: list) -> None:
...
def test_sort_by_date_returns_new_list(transactions: list) -> None:
